...
def continuous_variation(system,jacobian, u, M, t, dt):
    """Variation for continuous time systems."""
    _, dM = rk4var_step(system, jacobian, u, M, t, dt)
    M_next = M + dM
    return M_next
...
def calculate_LEs(save_fold,sys, sys_type, X, t, N_transient, dt, norm_step=1, target_dim=None):
    ...
    print('Dynamical system:',save_fold)
    if save_fold == 'VPT_L63':
        system = sys.solve_lorenz63
        jacobian = sys.solve_lorenz63_jac
    ...
    for i in range(1, N):
        if sys_type == "continuous":
            U = continuous_variation(system,jacobian, X[i], U, t[i], dt)
        # elif sys_type == "ESN":
        #     U = ESN_variation(system, X[i], X[i - 1], U)
